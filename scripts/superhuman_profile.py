...
#: This file lives at <skill_root>/scripts/superhuman_profile.py.
_SKILL_ROOT = Path(__file__).resolve().parent.parent

# So `from scripts.fleet... import ...` resolves when this script is run
# directly (not via `-m`) — the same seam other Claude-Code-adapter modules
# in this checkout already use, since this script is likewise invoked as a
# plain script (`scripts/superhuman_profile.py ...`), not as a package module.
...
#: Valid `effort` values for a `models:` tier entry (roadmap#275). `n/a` is a
#: real answer for a model that ignores reasoning effort, not a gap — see
#: `adaptation/dispatch.md` "Tier -> model + effort".
MODEL_EFFORTS: tuple[str, ...] = ("low", "medium", "high", "xhigh", "max", "n/a")

#: Valid `raised_effort` values. A narrower set than :data:`MODEL_EFFORTS`:
#: `raised_effort` only ever fires for a role that opted into `<tier>+raised`
#: (adaptation/role-tiers.json), so `n/a` (a model that ignores effort) makes
#: no sense as a *raised* answer.
RAISED_EFFORTS: tuple[str, ...] = ("low", "medium", "high", "xhigh", "max")

_MODEL_ENTRY_KEYS = {"primary", "fallback", "effort", "raised_effort"}
...
#: Matches a top-level `delegation:` key line in a Hermes `config.yaml` — the
#: same shape as `_MODELS_KEY_RE`, reused by `_find_top_level_key_span` for
#: `merge_hermes_delegation` (roadmap#275).
_DELEGATION_KEY_RE = re.compile(r"^delegation:(?:\s|$)")

_APPROVER_RE = re.compile(r"^(human|self|human:[\w.-]+|agent:[\w.-]+)$")
_STABLE_TAG_RE = re.compile(r"^v?\d+\.\d+\.\d+$")

#: Claude Code tier-agent generation (`models install-agents --harness
#: claude-code`, roadmap#275). `tier-agents.json` maps dispatch class ->
#: subagent_type; `tier-agent.md.tpl` is the per-agent body template.
TIER_AGENTS_JSON_PATH = _SKILL_ROOT / "templates" / "agents" / "claude-code" / "tier-agents.json"
TIER_AGENT_TEMPLATE_PATH = _SKILL_ROOT / "templates" / "agents" / "claude-code" / "tier-agent.md.tpl"

#: Frontmatter line marking a `~/.claude/agents/*.md` file as generated by
#: `models install-agents`. Its presence is the ownership boundary: a
#: same-named file lacking it is a human's hand-written agent definition and
#: `install_claude_code_agents` refuses to overwrite it.
GENERATED_AGENT_MARKER = "generated-by: superhuman_profile.py models install-agents"
...
class Profile:
    ...
    Attributes:
        ...
        models: Tier -> ``{"primary": <alias>, "fallback": <alias> | None,
            "effort": <level> | (absent), "raised_effort": <level> | (absent)}``
            (roadmap#275). Normalized at load time (ADR-6): a legacy
            bare-string tier value (``most_capable: opus``) is read as
            ``{"primary": "opus", "fallback": None}`` with no ``effort``/
            ``raised_effort`` keys at all; an already-mapping value passes
            through unchanged, and ``effort``/``raised_effort`` are present
            only when the source YAML declared them. A reader must use
            ``entry.get("effort")`` (``None`` for both an absent key and an
            explicit ``null`` — the same "unconfigured" meaning as the
            :data:`MODEL_PLACEHOLDER` string), never ``entry["effort"]``.
            This keeps a pre-existing profile with no ``effort:`` line
            loading unchanged (backward compatible), rather than forcing
            every tier entry — including ones this module never wrote — to
            carry the new keys.
...
def _normalize_model_entry(value: Any, where: str) -> dict[str, str | None]:
    ...
    Returns:
        A mapping with ``primary`` and ``fallback`` keys, plus ``effort`` /
        ``raised_effort`` only when the source declared them (backward
        compatibility — see :class:`Profile`'s ``models`` docstring).

    Raises:
        ProfileError: If ``value`` is neither a non-empty string nor a mapping
            with a valid ``primary``/``fallback``/``effort``/``raised_effort``
            shape.
    """
    ...
    if isinstance(value, dict):
        ...
        result: dict[str, str | None] = {"primary": primary, "fallback": fallback}
        if "effort" in value:
            effort = value["effort"]
            allowed = (*MODEL_EFFORTS, MODEL_PLACEHOLDER)
            if not isinstance(effort, str) or effort not in allowed:
                raise ProfileError(
                    f"{where}.effort: must be one of {list(allowed)}, got {effort!r}"
                )
            result["effort"] = effort
        if "raised_effort" in value:
            raised = value["raised_effort"]
            allowed_raised = (*RAISED_EFFORTS, MODEL_PLACEHOLDER)
            if not isinstance(raised, str) or raised not in allowed_raised:
                raise ProfileError(
                    f"{where}.raised_effort: must be one of {list(allowed_raised)}, "
                    f"got {raised!r}"
                )
            result["raised_effort"] = raised
        return result
...
def _find_top_level_key_span(lines: list[str], key_re: re.Pattern[str]) -> tuple[int, int] | None:
    """Find the line-index span of a top-level ``<key>:`` block in raw YAML text.

    Generalized from the ``models:``-specific span finder below (roadmap#275
    reused this for the Hermes ``delegation:`` block —
    :func:`_find_delegation_span`) — the scan logic is identical for any
    column-0 key, only the key regex differs.

    Args:
        lines: File content split with ``str.splitlines(keepends=True)``.
        key_re: Compiled pattern matching the target column-0 key line.

    Returns:
        ``(start, end)`` line indices with ``end`` exclusive, or ``None`` if
        no matching column-0 line is present.
    """
    start = None
    ...
    while i < len(lines):
        ...
        if line[:1] in (" ", "\t"):
            i += 1
            continue  # indented — a child of the mapping
...
def _find_models_span(lines: list[str]) -> tuple[int, int] | None:
    ...
    """
    return _find_top_level_key_span(lines, _MODELS_KEY_RE)
    ...
    *,
    ...
    Args:
        ...
        answers: Tier -> ``{"primary": <alias>, "fallback": <alias> | None,
            "effort": <level> | None, "raised_effort": <level> | None}`` for
            tiers the operator answered. ``None`` (the default) answers none.
            Unlike ``primary`` (which always resolves to a real value or
            :data:`MODEL_PLACEHOLDER`), ``effort``/``raised_effort`` are
            written only when the answer actually supplies one (truthy) —
            an answer naming just ``primary``/``fallback`` gets no
            ``effort`` key at all, the same "unconfigured" shape a legacy
            profile with no ``effort:`` line already normalizes to. This
            keeps every existing caller of this function (and of
            ``models set``) byte-for-byte unchanged when it never mentions
            effort. A brand-new, never-before-seen tier that this call does
            not answer at all still gets an explicit ``effort:
            PROMPT_ME`` — see the ``decline``/absent-tier handling below.
        decline: ``True`` to decline every tier in :data:`MODEL_TIERS`, or an
            ...
            is written with :data:`MODEL_PLACEHOLDER` for ``primary``,
            ``fallback`` and ``effort`` — never a concrete vendor/model name
            or effort level (FR-10, the provider-agnostic immutable
            constraint).
    ...
    for tier in MODEL_TIERS:
        if tier in declined:
            models_block[tier] = {
                ...
                "fallback": MODEL_PLACEHOLDER,
                "effort": MODEL_PLACEHOLDER,
            }
            ...
            # `effort`/`raised_effort` are only written when the caller
            # actually supplied one — an answer that names just primary/
            # fallback (e.g. an older caller unaware of roadmap#275) gets no
            # `effort` key at all, the same "unconfigured" shape a legacy
            # profile already normalizes to (Profile.models docstring), not a
            # forced PROMPT_ME. A freshly-created, wholly untouched tier
            # (the `elif tier not in models_block` branch below) is the one
            # case that DOES force the placeholder — that tier was never
            # answered at all, so leaving `effort` absent would silently look
            # identical to "operator was asked and it doesn't apply".
            effort = entry.get("effort")
            if effort:
                new_entry["effort"] = effort
            raised_effort = entry.get("raised_effort")
            if raised_effort:
                new_entry["raised_effort"] = raised_effort
            models_block[tier] = new_entry
...
def cmd_models_set(args: argparse.Namespace) -> int:
    ...
    for tier, entry in parsed.items():
        if not isinstance(entry, dict):
            raise ProfileError(
                f"--answers-json.{tier}: must be an object with "
                "'primary'/'fallback'/'effort'/'raised_effort'"
            )
        answers[tier] = {
    ...
    for tier in MODEL_TIERS:
        ...
        if primary == MODEL_PLACEHOLDER or primary is None:
            ...
            if raised_effort:
                parts.append(f"raised_effort={raised_effort}")
            print(f"  {tier:<14} " + " ".join(parts))
    return EXIT_OK


# --------------------------------------------------------------------------- #
# models install-agents (roadmap#275): harness-side model+effort config
# --------------------------------------------------------------------------- #


def _dispatch_class_parts(dispatch_class: str) -> tuple[str, bool]:
    """Split a dispatch class into its tier and whether it is the `+raised` form.

    Args:
        dispatch_class: One of :data:`DISPATCH_CLASSES`
            (`adaptation/role-tiers.json`'s vocabulary).

    Returns:
        ``(tier, raised)`` — ``tier`` is a key of :data:`MODEL_TIERS`;
        ``raised`` is True for e.g. ``most_capable+raised``.
    """
    suffix = "+raised"
    if dispatch_class.endswith(suffix):
        return dispatch_class[: -len(suffix)], True
    return dispatch_class, False


_TIER_AGENT_DESCRIPTION_TPL = (
    "Superhuman tier agent for dispatch class '{cls}', generated from "
    "~/.superhuman/profile.yaml by `scripts/superhuman_profile.py models "
    "install-agents`. Carries only a model and reasoning effort — the role for "
    "a given dispatch comes entirely from that dispatch's own prompt (the "
    "leading role block), never from this agent definition. Do not use "
    "directly outside superhuman dispatches."
)


@dataclass(frozen=True, slots=True)
class TierAgentPlan:
    """What `install_claude_code_agents` would do for one dispatch class.

    A pure, side-effect-free plan — computed once and consumed by both the
    installer (which writes it) and `doctor` (which only compares against it).

    Attributes:
        dispatch_class: The dispatch class this plan is for.
        agent_name: The `subagent_type` (from `tier-agents.json`).
        content: The rendered agent-definition file content, or ``None`` when
            this class is skipped (unconfigured tier, or `+raised` with no
            `raised_effort`).
        note: ``None``, or an explanation — a skip reason when ``content`` is
            ``None``, or a non-fatal warning (e.g. "effort unconfigured") when
...
def plan_claude_code_tier_agents(
    profile: Profile, class_map: dict[str, str], template_text: str
) -> list[TierAgentPlan]:
    """Compute what each dispatch class's tier-agent file should contain.

    Args:
        profile: The loaded operator profile.
        class_map: Dispatch class -> `subagent_type`, from
            :func:`scripts.fleet.role_tiers.load_harness_class_map`.
        template_text: Contents of `tier-agent.md.tpl`.

    Returns:
        One :class:`TierAgentPlan` per class in :data:`DISPATCH_CLASSES`.
    """
    plans: list[TierAgentPlan] = []
    for dispatch_class in DISPATCH_CLASSES:
        tier, raised = _dispatch_class_parts(dispatch_class)
        agent_name = class_map[dispatch_class]
        entry = profile.models.get(tier) or {}
        primary = entry.get("primary")

        if not primary or primary == MODEL_PLACEHOLDER:
            plans.append(TierAgentPlan(
                dispatch_class, agent_name, None,
                f"tier '{tier}' primary is unconfigured ({MODEL_PLACEHOLDER})",
            ))
            continue
        ...
        if raised:
            raised_effort = entry.get("raised_effort")
            if not raised_effort or raised_effort == MODEL_PLACEHOLDER:
                plans.append(TierAgentPlan(
                    dispatch_class, agent_name, None,
                    f"tier '{tier}' has no raised_effort configured",
                ))
                continue
            effort: str | None = raised_effort
        else:
            raw_effort = entry.get("effort")
            if raw_effort in (None, MODEL_PLACEHOLDER):
                effort = None
                note = (
                    f"tier '{tier}' effort is unconfigured ({MODEL_PLACEHOLDER}) — "
                    f"{agent_name} generated with no effort line (inherits session effort)"
                )
            elif raw_effort == "n/a":
                effort = None  # a real answer — this tier ignores effort
            else:
                effort = raw_effort

        description = _TIER_AGENT_DESCRIPTION_TPL.format(cls=dispatch_class)
        content = (
            template_text
            .replace("{{NAME}}", agent_name)
            .replace("{{DESCRIPTION}}", description)
            .replace("{{MODEL}}", primary)
            .replace("{{EFFORT_LINE}}", f"effort: {effort}\n" if effort else "")
            .replace("{{GENERATED_MARKER}}", GENERATED_AGENT_MARKER)
        )
        plans.append(TierAgentPlan(dispatch_class, agent_name, content, note))
    return plans


def install_claude_code_agents(profile: Profile, agents_dir: Path, *, dry_run: bool) -> list[str]:
    """Render and write one Claude Code tier-agent definition per dispatch class.

    Args:
        profile: The loaded operator profile.
        agents_dir: Destination directory (typically `~/.claude/agents`).
        dry_run: When True, compute and report what would happen, write nothing.

    Returns:
        Human-readable report lines (one per class: skip/warning/wrote/would write).

    Raises:
        ProfileError: If `tier-agents.json` is malformed, or a destination
            file already exists without :data:`GENERATED_AGENT_MARKER` — this
            module never overwrites a hand-written agent definition.
    """
    try:
        class_map = load_harness_class_map(TIER_AGENTS_JSON_PATH)
    except RoleTierError as exc:
        raise ProfileError(str(exc)) from exc
    template_text = TIER_AGENT_TEMPLATE_PATH.read_text(encoding="utf-8")

    plans = plan_claude_code_tier_agents(profile, class_map, template_text)
    messages: list[str] = []
    to_write: dict[Path, str] = {}
    for plan in plans:
        if plan.content is None:
            messages.append(f"skip {plan.agent_name}: {plan.note}")
            continue
        if plan.note:
            messages.append(f"warning: {plan.note}")
        dest = agents_dir / f"{plan.agent_name}.md"
        if dest.is_file():
            existing = dest.read_text(encoding="utf-8", errors="replace")
            if GENERATED_AGENT_MARKER not in existing:
                raise ProfileError(
                    f"{dest}: exists and carries no {GENERATED_AGENT_MARKER!r} marker — "
                    "refusing to overwrite a hand-written agent definition"
                )
        to_write[dest] = plan.content

    if dry_run:
        for dest in to_write:
            messages.append(f"would write {dest.as_posix()}")
        return messages

    agents_dir.mkdir(parents=True, exist_ok=True)
    for dest, content in to_write.items():
        dest.write_text(content, encoding="utf-8")
        messages.append(f"wrote {dest.as_posix()}")
    return messages


# --------------------------------------------------------------------------- #
# models install-agents — Hermes `delegation:` block
# --------------------------------------------------------------------------- #


def _merge_hermes_delegation_ruamel(text: str, updates: dict[str, Any]) -> str:
    """Merge `updates` into `delegation:`, preserving the whole file via ruamel.yaml.

    Args:
        text: The current `config.yaml` content (``""`` for a new file).
        updates: Keys to set/overwrite inside `delegation:` (other existing
            `delegation:` keys, and everything outside it, are untouched).

    Returns:
        The full, round-tripped document text.
    """
...
def _merge_hermes_delegation_textsplice(text: str, updates: dict[str, Any]) -> str:
    """Merge `updates` into `delegation:` via a targeted text splice (no ruamel.yaml).

    Mirrors :func:`_splice_models_block`: only the `delegation:` key's own
    span (per :func:`_find_top_level_key_span`) is replaced — everything else
    in the file, comments included, passes through byte-identical. The
    trade-off against the ruamel.yaml path above: a comment sitting *inside*
    the existing `delegation:` block is lost, because that block is re-dumped
    via `yaml.safe_dump` rather than round-tripped.

    Args:
        text: The current `config.yaml` content (``""`` for a new file).
        updates: Keys to set/overwrite inside `delegation:`.

    Returns:
        The full document text with only the `delegation:` span touched.
    """
    lines = text.splitlines(keepends=True)
    span = _find_top_level_key_span(lines, _DELEGATION_KEY_RE)
    ...
    if span is not None:
        ...
        try:
            ...
            raise ProfileError(f"existing delegation: block is not valid YAML — {exc}") from exc
        if not isinstance(loaded, dict) or not isinstance(loaded.get("delegation"), dict):
            raise ProfileError("existing top-level 'delegation:' key is not a mapping")
        existing = dict(loaded["delegation"])

    merged = {**existing, **updates}
    rendered = yaml.safe_dump(
        {"delegation": merged}, default_flow_style=False, sort_keys=False, allow_unicode=True
    )
    if span is None:
        prefix = text
        if prefix and not prefix.endswith("\n"):
...
def merge_hermes_delegation(
    config_path: Path, updates: dict[str, Any], *, dry_run: bool
) -> tuple[str, str]:
    """Merge `updates` into a Hermes `config.yaml`'s top-level `delegation:` block.

    Prefers a comment-preserving `ruamel.yaml` round-trip when that package is
    importable in the running interpreter; otherwise falls back to a targeted
    text splice (see :func:`_merge_hermes_delegation_textsplice` for the one
    trade-off — comments *inside* an existing `delegation:` block). Every
    other top-level key in `config.yaml`, and every other `delegation:`
    sub-key not named in `updates`, is preserved either way.

    Args:
        config_path: The Hermes instance's `config.yaml`.
        updates: Keys to set/overwrite inside `delegation:` (e.g. `model`,
            `reasoning_effort`).
        dry_run: When True, compute and return the result without writing.

    Returns:
        ``(method, new_text)`` — ``method`` names which merge path ran (for a
        human-readable report); ``new_text`` is the full resulting document.

    Raises:
        ProfileError: If the existing `delegation:` block (text-splice path)
            or the merged result is not valid YAML.
    """
    text = config_path.read_text(encoding="utf-8") if config_path.is_file() else ""
    try:
        import ruamel.yaml  # type: ignore[import-not-found]  # noqa: F401

        new_text = _merge_hermes_delegation_ruamel(text, updates)
        method = "ruamel.yaml round-trip (comments preserved throughout)"
    except ImportError:
        new_text = _merge_hermes_delegation_textsplice(text, updates)
        method = (
            "targeted text splice (ruamel.yaml not installed — comments outside "
            "delegation: preserved; comments inside an existing delegation: block are not)"
        )

    try:
        yaml.safe_load(new_text)
    except yaml.YAMLError as exc:
        raise ProfileError(f"merged delegation: block failed to validate as YAML — {exc}") from exc

    if not dry_run:
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(new_text, encoding="utf-8")
    return method, new_text


def cmd_models_install_agents(args: argparse.Namespace) -> int:
    """Generate/merge the harness-side model+effort config from the profile.

    Claude Code: renders one tier-agent definition per dispatch class into
    `--agents-dir` (see :func:`install_claude_code_agents`). Hermes: merges
    the `standard` tier's primary model + effort into `--hermes-config`'s
    `delegation:` block (see :func:`merge_hermes_delegation`) — per
    `adaptation/dispatch.md` "Hermes — config-level fallback", every
    ...
    # harness == "hermes"
    if not args.hermes_config:
        raise ProfileError("models install-agents --harness hermes requires --hermes-config <path>")

    entry = profile.models.get("standard") or {}
    primary = entry.get("primary")
    if not primary or primary == MODEL_PLACEHOLDER:
        print(
    ...
    effort = entry.get("effort")
    if effort in (None, MODEL_PLACEHOLDER):
        print(
            "superhuman-profile: warning — tier 'standard' effort is unconfigured "
            f"({MODEL_PLACEHOLDER}); delegation.reasoning_effort left untouched"
        )
    elif effort != "n/a":
        updates["reasoning_effort"] = effort
    # effort == "n/a": a real answer (this tier ignores effort) — omit the key,
    # per adaptation/dispatch.md's "n/a -> omit" rule.

    method, new_text = merge_hermes_delegation(
        Path(args.hermes_config), updates, dry_run=args.dry_run
    )
    verb = "would merge" if args.dry_run else "merged"
    print(f"superhuman-profile: {verb} delegation: block into {args.hermes_config} via {method}")
    if args.dry_run:
...
def cmd_doctor(args: argparse.Namespace) -> int:
    ...
    print()
    agents_dir = Path(args.agents_dir) if getattr(args, "agents_dir", None) else (
        Path.home() / ".claude" / "agents"
    )
    print(f"  claude-code tier agents  {agents_dir.as_posix()}")
    agent_lines, agent_problems = _claude_code_tier_agent_status(profile, agents_dir)
...
def _claude_code_tier_agent_status(profile: Profile, agents_dir: Path) -> tuple[list[str], int]:
    """Report each dispatch class's tier-agent file status for `doctor`.

    Never raises: this runs even when the profile has no `models:` block at
    all (every class then reports ``n/a``), and any failure loading
    `tier-agents.json`/`tier-agent.md.tpl` degrades to a single explanatory
    line rather than aborting `doctor`.

    Args:
        profile: The loaded operator profile.
        agents_dir: Directory to check (typically `~/.claude/agents`).

    Returns:
        ``(lines, problem_count)`` — ``lines`` are ready to print; missing or
        stale agent files count as a problem, ``n/a``/``ok`` do not.
    """
    try:
        class_map = load_harness_class_map(TIER_AGENTS_JSON_PATH)
        template_text = TIER_AGENT_TEMPLATE_PATH.read_text(encoding="utf-8")
    except (RoleTierError, OSError) as exc:
        return [f"    cannot check ({exc})"], 0
    ...
    for plan in plans:
        dest = agents_dir / f"{plan.agent_name}.md"
        if plan.content is None:
            status = f"n/a ({plan.note})"
        elif not dest.is_file():
            status = "MISSING — run: models install-agents --harness claude-code"
            problems += 1
        else:
            actual = dest.read_text(encoding="utf-8", errors="replace")
            if actual == plan.content:
                status = "ok"
            else:
                status = "STALE — run: models install-agents --harness claude-code"
                problems += 1
        lines.append(f"    {plan.agent_name:<48} {status}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    doctor = add('doctor', 'one-screen health report for this location')
    doctor.add_argument(
        '--agents-dir',
        default=None,
        help='claude-code tier-agent directory to check (default: ~/.claude/agents)',
    )
    ...
    install_agents = models_subs.add_parser(
        "install-agents",
        help="generate/merge the harness-side model+effort config from the profile",
    )
    install_agents.add_argument(
        "--harness", required=True, choices=("claude-code", "hermes"),
        help="which harness to install for",
    )
    install_agents.add_argument(
    ...
    install_agents.add_argument(
        ...
        help="claude-code: destination for tier-agent .md files (default: ~/.claude/agents)",
    )
    install_agents.add_argument(
        "--hermes-config", default=None,
        help="hermes: the instance's config.yaml (required with --harness hermes)",
    )
    install_agents.add_argument(
        "--dry-run", action="store_true", help="report what would happen, write nothing",
    )
    install_agents.set_defaults(func=cmd_models_install_agents)
