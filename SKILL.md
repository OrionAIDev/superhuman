...
3. **Accepted is not complete, and only one of the two is yours to declare.** All 8 phase gates (G0, G1, G2, G3, G4, G5, G7, G8) having fired with their HITL approvals logged in SUPERHUMAN.md `## Decisions log` establishes that the project is **accepted** — built, verified, signed off. NEVER report that state as "complete", "done" or "finished". A project is **complete** only when it is accepted AND one of:
   - every rung in its declared `**Deployment ladder:**` carries a recorded `promote_into` sign-off. A ladder declared as `none` is satisfied immediately — an empty ladder has nothing left to clear; or
   - the human PM has **explicitly declared it complete** over an incomplete ladder. This override always stays available, and is never assumed: log it in `## Decisions log` like any other decision, naming the rungs left uncleared.

   Where `**Deployment ladder:**` is absent (a project predating the field), the ladder is **undeclared** — report it as undeclared. Never read an absent ladder as `none`, and never infer what the ladder should have been; ask the PM. Read these sign-off records, never re-derive them: the promotions underneath them are irreversible acts that already carry code plus an explicit human yes.
...
- **Model-tier routing.** Every dispatch resolves a model AND a reasoning effort — never inherited from the parent session. Use the cheapest tier per role: most-capable for PM, Architect, code-quality review (security review at most-capable, raised effort); standard for Developer, QA, Business Expert, surrogate-user; cheap for Tester, docs-sync, convention checks. A role may run at an opt-in class only with a `superhuman-tier-opt-in: <reason>` line in the task brief and a decisions-log entry. Authoritative table: `adaptation/role-tiers.json`; harness mapping (Claude Code tier agents, OpenClaw `thinking=`, Hermes fallback): `adaptation/dispatch.md`.
