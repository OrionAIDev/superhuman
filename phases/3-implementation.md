...
through `python -m scripts.fleet.cli observe handoff-emit --prompt-file ... --output-file ...` rather than handing it
...
resolves `scripts.fleet`. There is no bare `fleet` executable on `PATH`, and the skill root is not
interchangeable with `--workspace`, which names the project being observed.

## Per-chunk dispatch observation (non-gating)

After PM dispatches Developer for a chunk (Step 2 above), PM calls
`python -m scripts.fleet.cli observe dispatch --harness subagent --dispatch-id <role>-<chunk>-<n> --local-id
